...
export const load: PageServerLoad = async ({ params, locals }) => {
	...
	// Validate school if it exists in the document
	let normalizedSchool: string | null = null;
	if (document.school) {
		try {
			normalizedSchool = validateAndNormalizeSchool(document.school);
		} catch {
			// If validation fails, keep the original value but log it
			console.warn('Invalid school name in document:', document.school);
		}
	}
	...
	return {
		document: {
			...
			school: normalizedSchool || document.school, // Use normalized school if available
		},
